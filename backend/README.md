...
Core application tables follow the ERD audit-column convention `create_time`, `update_time`,
`creator`, and `updater`. Expense API responses expose the time values as `createTime` and
`updateTime`; `creator` and `updater` remain internal database audit fields.
...
- Use the audit columns `create_time`, `update_time`, `creator`, and `updater` for core
  application tables, and update stored procedures so mutations maintain them.
